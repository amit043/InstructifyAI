...
class ModelRoute(Base):
    ...
    __table_args__ = (
        sa.UniqueConstraint(
            "project_id", "doc_id", "adapter_id", name="uq_model_routes_key"
        ),
        sa.Index("ix_model_routes_project_doc", "project_id", "doc_id"),
    )
...
def register_model_route(
    ...
    *,
    ...
    if did is None:
        stmt = stmt.where(ModelRoute.doc_id.is_(None))
    else:
        stmt = stmt.where(ModelRoute.doc_id == did)
    ...
    route = ModelRoute(project_id=pid, adapter_id=aid, doc_id=did)
    db.add(route)
    db.commit()
    db.refresh(route)
    return route
...
def resolve_model_routes(
    ...
    *,
    ...
    project_routes: Sequence[ModelRoute] = db.scalars(
        sa.select(ModelRoute)
        .where(ModelRoute.project_id == pid, ModelRoute.doc_id.is_(None))
        .order_by(ModelRoute.created_at.desc())
    ).all()
